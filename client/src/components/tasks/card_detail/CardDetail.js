  ...
  const handleAddMemberFormAdd = async () => {
    if (newMember.trim()) {
        try {
            ...
            if (assignedUserData) {
                await assignUserToCard(boardId, listId, taskId, assignedUserData._id);
    ...
    const fetchCardDetails = async () => {
      try {
          ...
          if (cardDetails && cardDetails.card_assignees) {
              const fetchedMembers = cardDetails.card_assignees.map(assignee => {
  ...
  const handleMemberInputChange = (e) => {
    ...
    if (inputValue.trim() && memberSuggestions) {
      const filteredSuggestions = memberSuggestions.filter(member =>
        (member.user_email || '').toLowerCase().includes(inputValue.toLowerCase())
      );
...
<div className="member-icons-container">
    {members.length > 0 && members.map((member, index) => (
        member && <div key={index} className={`member-icon member-icon-${index + 1}`}>
            {member.charAt(0).toUpperCase()}
        </div>
    ))}
    <div className="add-member-icon" onClick={handleAddMemberIconClick}>+</div>
            ...
            <div className="add-member-list">
              {memberSuggestions.map(member => (
                <div
                  ...
                  onClick={() => handleMemberSuggestionClick(member)}
                >
                  {member.user_email} {/* Display user_email here */}
                </div>
              ))}
            </div>
            <div className="add-member-actions">
              <button className="add-member-cancel" onClick={handleAddMemberFormCancel}>CANCEL</button>
              <button className="add-member-add" onClick={handleAddMemberFormAdd}>ADD</button>
            </div>
          </div>
        </div>
      )}
