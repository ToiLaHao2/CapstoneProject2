...
//     // Lọc danh sách boards dựa trên globalSearchQuery
//     const filteredBoards = boards.filter(board =>
//         board.board_title.toLowerCase().includes(globalSearchQuery.toLowerCase())
//     );
...
const Projects = () => {
    ...
    const [userMenuVisibleLocation, setUserMenuVisibleLocation] = useState({ boardId: null, userId: null });
    const userMenuRef = useRef({});
    const currentUserMenuRef = useRef(null); 
    ...
    const toggleUserMenu = (e, boardId, userId) => {
        e.stopPropagation();
        console.log("Toggling user menu for boardId:", boardId, "userId:", userId);

        if (userMenuVisibleLocation.boardId === boardId && userMenuVisibleLocation.userId === userId) {
            setUserMenuVisibleLocation({ boardId: null, userId: null }); 
        } else {
            setUserMenuVisibleLocation({ boardId: boardId, userId: userId }); 
        }
        closeMenu(); 
    };
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            if (userMenuVisibleLocation.boardId && userMenuVisibleLocation.userId) {
                if (currentUserMenuRef.current && !currentUserMenuRef.current.contains(event.target)) {
                    closeUserMenu();
                }
            }
        };
    ...
    }, [menuVisibleProjectId, userMenuVisibleLocation]);
                        ...
                        >
                            <div className="project-card-top">
                                ...
                                <div className="project-status-and-users">
                                    ...
                                    {/* Conditionally render user icons */}
                                    <div className="project-users-icons">
                                        {project.board_collaborators.map((collaborator, collabIndex) => { // SỬA: Đổi index thành collabIndex để tránh nhầm lẫn
                                            // console.log("Inside map - collaborator:", collaborator);
                                            // collaborator._boardId = project._id; // Không cần thiết gán ở đây, truyền trực tiếp vào hàm toggle

                                            return (
                                                <React.Fragment key={`${project._id}-${collaborator._id}`}> {/* SỬA: Key unique hơn */}
                                                    {collaborator?.user_avatar_url !== "empty" ? (
                                                        <img
                                                            className="user-icon-circle"
                                                            src={collaborator.user_avatar_url}
                                                            alt="small-avatar"
                                                            onClick={(e) => {
                                                                // SỬA: Truyền project._id và collaborator._id
                                                                toggleUserMenu(e, project._id, collaborator._id);
                                                            }}
                                                        />
                                                    ) : (
                                                        <div
                                                    ...
                                                    {/* SỬA: Điều kiện render menu dropdown */}
                                                    {userMenuVisibleLocation.boardId === project._id && userMenuVisibleLocation.userId === collaborator?._id && (
                                                        <div
                                                            className="dropdown-menu user-dropdown"
                                                            ref={(el) => (userMenuRef.current[`${project._id}-${collaborator?._id}`] = el)} // SỬA: Key cho ref
                                                            onClick={(e) => e.stopPropagation()} // Ngăn chặn click menu con đóng menu cha
                                                        >
                                                            <div className="dropdown-item">
                                                                <b>Member: {collaborator?.user_full_name}</b>
                                                            </div>
                                                            {/* Kiểm tra nếu collaborator.user_id khác với user._id của người đang đăng nhập mới hiện nút remove */}
                                                            {collaborator._id !== user._id && (
                                                                <div className="dropdown-item" onClick={() => handleRemoveMemberClick({ ...collaborator, _boardId: project._id })}> {/* Truyền đầy đủ thông tin để remove */}
                                                                    Remove from board
                                                                </div>
                                                            )}
                                                        </div>
